...
@section('breadcrumbs')
<li class="breadcrumb-item active" aria-current="page">Dorms</li>
@endsection
@section('pageTitle')
<h1>Dorms</h1>
@endsection
...
@endsection
@section('content')
<div class="card dorms_index admikoIndex">
    <div class="card-body">
        <div class="tableBox" id="tableBox">
            ...
            <div class="tableLayout pb-2">
                    ...
                    <thead>
                        <tr data-sort-method='thead'>
							...
							<th scope="col" class="text-nowrap">Dorm Name</th>
							<th scope="col" class="text-nowrap d-none d-sm-table-cell">Dorm Type</th>
							<th scope="col" class="text-nowrap d-none d-md-table-cell">Description</th>
                            <th scope="col" class="w-5 no-sort" data-orderable="false">{{trans("admiko.table_edit")}}</th>
                            @if(Gate::allows('dorms_allow'))
                    ...
                    @foreach($tableData as $data)
                        <tr>
							<td class="w-5"><a href="{{route("admin.dorms.edit",[$data->id])}}">{{$data->id}}</a></td>
							<td class="text-nowrap">{{$data->dorm_name}}</td>
							<td class="text-nowrap d-none d-sm-table-cell">{{$data->dorm_type_id->dorm_type??""}}</td>
							<td class="text-nowrap d-none d-md-table-cell">{{$data->description}}</td>
                            <td class="w-5 no-sort"><a href="{{route("admin.dorms.edit",[$data->id])}}"><i class="fas fa-edit fa-fw"></i></a></td>
                            @if(Gate::allows(['dorms_allow']))
            ...
            <div class="row">
                <div class="col-12 col-sm order-3 order-sm-0 pt-2">
                    @if(Gate::any(['dorms_allow']))
                        <a href="{{route('admin.dorms.create')}}" class="btn btn-primary" role="button"><i class="fas fa-plus fa-fw"></i> {{trans('admiko.table_add')}}</a>
                    @endIf
                </div>
    ...
    <div class="modal fade" id="deleteConfirm" tabindex="-1" role="dialog" aria-labelledby="deleteConfirm" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form method="post" class="w-100" action="{{route("admin.dorms.delete")}}">
            @method('DELETE')
            @csrf
