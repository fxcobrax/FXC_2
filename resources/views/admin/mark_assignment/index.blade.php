...
@section('breadcrumbs')
<li class="breadcrumb-item active" aria-current="page">Mark Assignment</li>
@endsection
@section('pageTitle')
...
@endsection
@section('content')
<div class="card mark_assignment_index admikoIndex">
    <div class="card-body">
        <div class="tableBox" id="tableBox">
            ...
            <div class="tableLayout pb-2">
                    ...
                    <thead>
                        <tr data-sort-method='thead'>
							...
							<th scope="col" class="text-nowrap d-none d-md-table-cell">Marks</th>
                            ...
                            @if(Gate::allows('mark_assignment_allow'))
                    ...
                    @foreach($tableData as $data)
                        <tr>
							<td class="w-5"><a href="{{route("admin.mark_assignment.edit",[$data->id])}}">{{$data->id}}</a></td>
							<td class="text-nowrap">{{$data->assignment_name_id->assignment_name??""}}</td>
							<td class="text-nowrap d-none d-sm-table-cell">{{$data->student_name_id->first_name??""}}</td>
							<td class="text-nowrap d-none d-md-table-cell">{{$data->marks}}</td>
							<td class="d-none d-lg-table-cell">{{$data->date}}</td>
                            <td class="w-5 no-sort"><a href="{{route("admin.mark_assignment.edit",[$data->id])}}"><i class="fas fa-edit fa-fw"></i></a></td>
                            @if(Gate::allows(['mark_assignment_allow']))
            ...
            <div class="row">
                <div class="col-12 col-sm order-3 order-sm-0 pt-2">
                    @if(Gate::any(['mark_assignment_allow']))
                        <a href="{{route('admin.mark_assignment.create')}}" class="btn btn-primary" role="button"><i class="fas fa-plus fa-fw"></i> {{trans('admiko.table_add')}}</a>
                    @endIf
                </div>
    ...
    <div class="modal fade" id="deleteConfirm" tabindex="-1" role="dialog" aria-labelledby="deleteConfirm" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form method="post" class="w-100" action="{{route("admin.mark_assignment.delete")}}">
            @method('DELETE')
            @csrf
