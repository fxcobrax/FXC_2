...
@endsection
@section('content')
<div class="card item_category_index admikoIndex">
    <div class="card-body">
        <div class="tableBox" id="tableBox">
            ...
            <div class="tableLayout pb-2">
                    ...
                    <thead>
                        <tr data-sort-method='thead'>
                            ...
                            @if(Gate::allows('item_category_allow'))
                    ...
                    @foreach($tableData as $data)
                        <tr>
							<td class="w-5"><a href="{{route("admin.item_category.edit",[$data->id])}}">{{$data->id}}</a></td>
							<td class="text-nowrap">{{$data->category_name}}</td>
							<td class="text-nowrap d-none d-sm-table-cell">{{$data->description}}</td>
                            <td class="w-5 no-sort"><a href="{{route("admin.item_category.edit",[$data->id])}}"><i class="fas fa-edit fa-fw"></i></a></td>
                            @if(Gate::allows(['item_category_allow']))
            ...
            <div class="row">
                <div class="col-12 col-sm order-3 order-sm-0 pt-2">
                    @if(Gate::any(['item_category_allow']))
                        <a href="{{route('admin.item_category.create')}}" class="btn btn-primary" role="button"><i class="fas fa-plus fa-fw"></i> {{trans('admiko.table_add')}}</a>
                    @endIf
                </div>
    ...
    <div class="modal fade" id="deleteConfirm" tabindex="-1" role="dialog" aria-labelledby="deleteConfirm" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form method="post" class="w-100" action="{{route("admin.item_category.delete")}}">
            @method('DELETE')
            @csrf
