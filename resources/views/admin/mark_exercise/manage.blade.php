...
@section('breadcrumbs')
    <li class="breadcrumb-item active"><a href="{{ route("admin.mark_exercise.index") }}">Mark Exercise</a></li>
    @if(isset($data))
...
@endsection
@section('backBtn')
<a href="{{ route("admin.mark_exercise.index") }}"><i class="fas fa-angle-left"></i> {{trans('admiko.page_back_btn')}}</a>
@endsection
@section('content')
<div class="card formPage mark_exercise_manage admikoForm">
            ...
            <div class="row">
                ...
                <div class=" col-12">
                    <div class="form-group row multiSelect">
                        ...
                        <div class="col-md-10" style="position: relative">
                            <select class="form-select" id="exercise_name" name="exercise_name" required="true" data-placeholder="{{trans('admiko.select')}}" style="width: 100%" data-width="100%" data-allow-clear="true">
                                
                                @foreach($exercise_all as $id => $value)
                                    <option value="{{ $id }}" {{ (old('exercise_name') ? old('exercise_name') : $data->exercise_name ?? '') == $id ? 'selected' : '' }}>{{ $value }}</option>
                                @endforeach
                            </select>
        ...
        <div class="card-footer form-actions" id="form-group-buttons">
            <div class="row">
                ...
                <div class="col-sm-10">
                    ...
                    <a href="{{ route("admin.mark_exercise.index") }}" class="btn btn-secondary float-end" role="button">{{trans('admiko.table_cancel')}}</a>
                </div>
            </div>
        </div>
    </form>
