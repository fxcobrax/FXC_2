...
@section('breadcrumbs')
    <li class="breadcrumb-item active"><a href="{{ route("admin.mark_assignment.index") }}">Mark Assignment</a></li>
    @if(isset($data))
...
@endsection
@section('backBtn')
<a href="{{ route("admin.mark_assignment.index") }}"><i class="fas fa-angle-left"></i> {{trans('admiko.page_back_btn')}}</a>
@endsection
@section('content')
<div class="card formPage mark_assignment_manage admikoForm">
                ...
                <div class=" col-12">
                    <div class="form-group row multiSelect">
                        ...
                        <div class="col-md-10" style="position: relative">
                            <select class="form-select" id="assignment_name" name="assignment_name" required="true" data-placeholder="{{trans('admiko.select')}}" style="width: 100%" data-width="100%" data-allow-clear="true">
                                
                                @foreach($assignment_all as $id => $value)
                                    <option value="{{ $id }}" {{ (old('assignment_name') ? old('assignment_name') : $data->assignment_name ?? '') == $id ? 'selected' : '' }}>{{ $value }}</option>
                                @endforeach
                            </select>
        ...
        <div class="card-footer form-actions" id="form-group-buttons">
            <div class="row">
                ...
                <div class="col-sm-10">
                    ...
                    <a href="{{ route("admin.mark_assignment.index") }}" class="btn btn-secondary float-end" role="button">{{trans('admiko.table_cancel')}}</a>
                </div>
            </div>
        </div>
    </form>
