...
@endsection
@section('content')
<div class="card students_list_index admikoIndex">
    <div class="card-body">
        <div class="tableBox" id="tableBox">
            ...
            <div class="tableLayout pb-2">
                    ...
                    <thead>
                        <tr data-sort-method='thead'>
							...
							<th scope="col" class="text-nowrap d-none d-md-table-cell">Phone Number</th>
							<th scope="col" class="text-nowrap d-none d-lg-table-cell">Class</th>
							<th scope="col" class="text-nowrap d-none d-lg-table-cell">Department</th>
                            <th scope="col" class="w-5 no-sort" data-orderable="false">{{trans("admiko.table_edit")}}</th>
                            @if(Gate::allows('students_list_allow'))
                    ...
                    @foreach($tableData as $data)
                        <tr>
							<td class="w-5"><a href="{{route("admin.students_list.edit",[$data->id])}}">{{$data->id}}</a></td>
							<td>@if (isset($data->student_picture) && Storage::disk(config("admiko_config.filesystem"))->exists($admiko_data["fileInfo"]["student_picture"]["original"]["folder"].$data->student_picture))
							...
							<td class="text-nowrap d-none d-sm-table-cell">{{$data->first_name}}</td>
							<td class="text-nowrap d-none d-md-table-cell">{{$data->phone_number}}</td>
							<td class="text-nowrap d-none d-lg-table-cell">{{$data->select_class_id->class_name??""}}</td>
							<td class="text-nowrap d-none d-lg-table-cell">{{$data->select_department_id->dept_name??""}}</td>
                            <td class="w-5 no-sort"><a href="{{route("admin.students_list.edit",[$data->id])}}"><i class="fas fa-edit fa-fw"></i></a></td>
                            @if(Gate::allows(['students_list_allow']))
            ...
            <div class="row">
                <div class="col-12 col-sm order-3 order-sm-0 pt-2">
                    @if(Gate::any(['students_list_allow']))
                        <a href="{{route('admin.students_list.create')}}" class="btn btn-primary" role="button"><i class="fas fa-plus fa-fw"></i> {{trans('admiko.table_add')}}</a>
                    @endIf
                </div>
    ...
    <div class="modal fade" id="deleteConfirm" tabindex="-1" role="dialog" aria-labelledby="deleteConfirm" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form method="post" class="w-100" action="{{route("admin.students_list.delete")}}">
            @method('DELETE')
            @csrf
