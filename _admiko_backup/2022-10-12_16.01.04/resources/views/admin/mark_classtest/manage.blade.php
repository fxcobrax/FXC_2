...
@section('breadcrumbs')
    <li class="breadcrumb-item active"><a href="{{ route("admin.mark_classtest.index") }}">Mark Classtest</a></li>
    @if(isset($data))
...
@endsection
@section('pageTitle')
<h1>Mark Classtest</h1>
@endsection
@section('pageInfo')
@endsection
@section('backBtn')
<a href="{{ route("admin.mark_classtest.index") }}"><i class="fas fa-angle-left"></i> {{trans('admiko.page_back_btn')}}</a>
@endsection
@section('content')
<div class="card formPage mark_classtest_manage admikoForm">
            ...
            <div class="row">
                ...
                <div class=" col-12">
                    <div class="form-group row multiSelect">
                        <label for="test_name" class="col-md-2 col-form-label">Test Name:*</label>
                        <div class="col-md-10" style="position: relative">
                            <select class="form-select" id="test_name" name="test_name" required="true" data-placeholder="{{trans('admiko.select')}}" style="width: 100%" data-width="100%" data-allow-clear="true">
                                
                                @foreach($classtest_all as $id => $value)
                                    <option value="{{ $id }}" {{ (old('test_name') ? old('test_name') : $data->test_name ?? '') == $id ? 'selected' : '' }}>{{ $value }}</option>
                                @endforeach
                            </select>
                            <div class="invalid-feedback @if ($errors->has('test_name')) d-block @endif">{{trans('admiko.required_text')}}</div>
                            <small id="test_name_help" class="text-muted"></small>
                        </div>
                    </div>
                </div>
        ...
        <div class="card-footer form-actions" id="form-group-buttons">
            <div class="row">
                ...
                <div class="col-sm-10">
                    ...
                    <a href="{{ route("admin.mark_classtest.index") }}" class="btn btn-secondary float-end" role="button">{{trans('admiko.table_cancel')}}</a>
                </div>
            </div>
        </div>
    </form>
