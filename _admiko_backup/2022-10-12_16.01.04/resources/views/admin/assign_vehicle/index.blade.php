...
@endsection
@section('content')
<div class="card assign_vehicle_index admikoIndex">
    <div class="card-body">
        <div class="tableBox" id="tableBox">
            ...
            <div class="tableLayout pb-2">
                    ...
                    <thead>
                        <tr data-sort-method='thead'>
							...
							<th scope="col" class="text-nowrap d-none d-sm-table-cell">Driver</th>
                            <th scope="col" class="w-5 no-sort" data-orderable="false">{{trans("admiko.table_edit")}}</th>
                            @if(Gate::allows('assign_vehicle_allow'))
                    ...
                    @foreach($tableData as $data)
                        <tr>
							<td class="w-5"><a href="{{route("admin.assign_vehicle.edit",[$data->id])}}">{{$data->id}}</a></td>
							<td class="text-nowrap">{{$data->vehicle_id_id->vehicle_id??""}}</td>
							<td class="text-nowrap d-none d-sm-table-cell">{{$data->driver_id->full_name??""}}</td>
                            <td class="w-5 no-sort"><a href="{{route("admin.assign_vehicle.edit",[$data->id])}}"><i class="fas fa-edit fa-fw"></i></a></td>
                            @if(Gate::allows(['assign_vehicle_allow']))
            ...
            <div class="row">
                <div class="col-12 col-sm order-3 order-sm-0 pt-2">
                    @if(Gate::any(['assign_vehicle_allow']))
                        <a href="{{route('admin.assign_vehicle.create')}}" class="btn btn-primary" role="button"><i class="fas fa-plus fa-fw"></i> {{trans('admiko.table_add')}}</a>
                    @endIf
                </div>
    ...
    <div class="modal fade" id="deleteConfirm" tabindex="-1" role="dialog" aria-labelledby="deleteConfirm" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form method="post" class="w-100" action="{{route("admin.assign_vehicle.delete")}}">
            @method('DELETE')
            @csrf
