...
@section('breadcrumbs')
<li class="breadcrumb-item active" aria-current="page">Sign Exeat</li>
@endsection
@section('pageTitle')
<h1>Sign Exeat</h1>
@endsection
...
@endsection
@section('content')
<div class="card signexeat1_index admikoIndex">
    <div class="card-body">
        <div class="tableBox" id="tableBox">
            ...
            <div class="tableLayout pb-2">
                    ...
                    <thead>
                        <tr data-sort-method='thead'>
                            ...
                            @if(Gate::allows('signexeat1_allow'))
                    ...
                    @foreach($tableData as $data)
                        <tr>
							<td class="w-5"><a href="{{route("admin.signexeat1.edit",[$data->id])}}">{{$data->id}}</a></td>
							<td class="text-nowrap">{{$data->student_name_id->first_name??""}}</td>
							<td class="d-none d-sm-table-cell">{{$data->from}}</td>
							<td class="d-none d-md-table-cell">{{$data->to}}</td>
							<td class="d-none d-lg-table-cell">{!!Str::limit(strip_tags($data["reason"]), 50, "...")!!}</td>
                            <td class="w-5 no-sort"><a href="{{route("admin.signexeat1.edit",[$data->id])}}"><i class="fas fa-edit fa-fw"></i></a></td>
                            @if(Gate::allows(['signexeat1_allow']))
            ...
            <div class="row">
                <div class="col-12 col-sm order-3 order-sm-0 pt-2">
                    @if(Gate::any(['signexeat1_allow']))
                        <a href="{{route('admin.signexeat1.create')}}" class="btn btn-primary" role="button"><i class="fas fa-plus fa-fw"></i> {{trans('admiko.table_add')}}</a>
                    @endIf
                </div>
    ...
    <div class="modal fade" id="deleteConfirm" tabindex="-1" role="dialog" aria-labelledby="deleteConfirm" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form method="post" class="w-100" action="{{route("admin.signexeat1.delete")}}">
            @method('DELETE')
            @csrf
