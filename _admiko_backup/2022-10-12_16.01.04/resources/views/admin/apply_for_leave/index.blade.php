...
@endsection
@section('content')
<div class="card apply_for_leave_index admikoIndex">
    <div class="card-body">
        <div class="tableBox" id="tableBox">
            ...
            <div class="tableLayout pb-2">
                    ...
                    <thead>
                        <tr data-sort-method='thead'>
							...
							<th scope="col" class="text-nowrap">Staff Name</th>
							<th scope="col" class="text-nowrap d-none d-sm-table-cell">Leave Type</th>
							<th scope="col" class="d-none d-md-table-cell">From</th>
							<th scope="col" class="d-none d-lg-table-cell">To</th>
                            ...
                            @if(Gate::allows('apply_for_leave_allow'))
                    ...
                    @foreach($tableData as $data)
                        <tr>
							<td class="w-5"><a href="{{route("admin.apply_for_leave.edit",[$data->id])}}">{{$data->id}}</a></td>
							<td class="text-nowrap">{{$data->staff_name_id->full_name??""}}</td>
							<td class="text-nowrap d-none d-sm-table-cell">{{$data->leave_type_id->type_name??""}}</td>
							<td class="d-none d-md-table-cell">{{$data->from}}</td>
							<td class="d-none d-lg-table-cell">{{$data->to}}</td>
							<td class="d-none d-lg-table-cell">{!!Str::limit(strip_tags($data["reason"]), 50, "...")!!}</td>
                            <td class="w-5 no-sort"><a href="{{route("admin.apply_for_leave.edit",[$data->id])}}"><i class="fas fa-edit fa-fw"></i></a></td>
                            @if(Gate::allows(['apply_for_leave_allow']))
            ...
            <div class="row">
                <div class="col-12 col-sm order-3 order-sm-0 pt-2">
                    @if(Gate::any(['apply_for_leave_allow']))
                        <a href="{{route('admin.apply_for_leave.create')}}" class="btn btn-primary" role="button"><i class="fas fa-plus fa-fw"></i> {{trans('admiko.table_add')}}</a>
                    @endIf
                </div>
    ...
    <div class="modal fade" id="deleteConfirm" tabindex="-1" role="dialog" aria-labelledby="deleteConfirm" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form method="post" class="w-100" action="{{route("admin.apply_for_leave.delete")}}">
            @method('DELETE')
            @csrf
